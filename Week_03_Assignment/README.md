# Week 03 Assignment (Superstore Advanced SQL Analysis)
...
- `Superstore_Customer_Sales_Insights.sql` — Complete SQL file covering all steps: schema setup, required queries, final combined query, and mini project
...
`RANK()` is applied inside a subquery to rank customers by total sales. The outer query then filters to only `sales_rank <= 3`. Note: if multiple customers are tied at rank 3, all of them are returned.
